...
public class OrderService(ApplicationDbContext dbContext, ILogger<OrderService> logger)
    ...
    protected override IQueryable<Order> CreateDbSource(IQueryable<Order> unjoinedSource) {
        return unjoinedSource.Include(x => x.OrderDetails);
    }
    ...
    public async Task<bool> UpdateAsync(OrderEditor editor) {
        ExceptionUtils.ThrowIfNull(() => editor);
        if (editor.Id is not Guid id) {
            throw new ArgumentNullException(nameof(editor.Id));
        }

        Order? entity = await DbContext.Orders.SingleOrDefaultAsync(x => x.Id == id);

        ExceptionUtils.ThrowIfItemNotFound(entity);

        return await UseTransactionAsync(async (transaction) => {
            IQueryable<OrderDetail> detailEntities = DbContext.OrderDetails.Where(x => x.OrderId == id);
    ...
    public async Task<DateTime?> GetDateAsync(Guid id) {
