...
public static class OrderingSystemAutoMapper {
    private static readonly Lock syncRoot = new();
    private static string? configuredLicenseKey;
    private static ILoggerFactory? configuredLoggerFactory;
    private static readonly Lazy<IMapper> mapper = new(() => {
        MapperConfiguration config = new(cfg => {
            if (!string.IsNullOrWhiteSpace(configuredLicenseKey)) {
                cfg.LicenseKey = configuredLicenseKey;
            }
            cfg.AddProfile(new ServiceProfile());
        }, configuredLoggerFactory ?? NullLoggerFactory.Instance);
        config.AssertConfigurationIsValid();
        return config.CreateMapper();
    });

    public static void Configure(string? licenseKey = null, ILoggerFactory? loggerFactory = null) {
        if (mapper.IsValueCreated) {
            return;
        }
        ...
        lock (syncRoot) {
            ...
            configuredLicenseKey = string.IsNullOrWhiteSpace(licenseKey)
                ? Environment.GetEnvironmentVariable("AUTOMAPPER_LICENSE_KEY")
                : licenseKey;
            configuredLoggerFactory = loggerFactory;
        }
    }
...
public abstract class ServiceBase<TService> : IDomainService, IScopedDependency
    ...
    protected IMapper Mapper => OrderingSystemAutoMapper.Mapper;
